...
const normalizeItem = (it: DepthCarouselItem) => {
  if (typeof it === 'string') return { image: it, alt: '', content: null as ReactNode }
  if ('content' in it) return { image: '', alt: it.alt ?? '', content: it.content as ReactNode }
  return { image: it.image, alt: it.alt ?? '', content: null as ReactNode }
}
        ...
        {data.map((item, i) => (
          <div
            ...
            style={{
              ...
              height: cardHeight,
              borderRadius: radius,
              background: item.content ? 'var(--bg-card)' : '#0b0d12',
              border: item.content ? '1px solid var(--glass-border)' : 'none',
            }}
          ...
          >
            {item.content ? (
              <div className="w-full h-full overflow-y-auto">{item.content}</div>
            ) : (
              <img
